...
import { Award, RefreshCw, Zap, TrendingUp, Filter, Play, Pause } from 'lucide-react';
...
export default function LeadLeaderboard({
  ...
  useEffect(() => {
    ...
    const slideInterval = setInterval(() => {
      const currentIndex = filterCodes.indexOf(selectedApparatus);
      const nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % filterCodes.length;
      onFilterChange(filterCodes[nextIndex]);
    }, 7000);

    return () => clearInterval(slideInterval);
  }, [isSlideMode, selectedApparatus, onFilterChange]);
  ...
  return (
    <div id="live-scoreboard-container" className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm transition-all duration-300 flex flex-col">
      ...
      <div className="p-5 border-b border-slate-200 bg-slate-50 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {/* Caching/Stream metadata and slide controls */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-2xs font-mono text-slate-500 self-start md:self-auto">
          <div className="flex items-center gap-2">
            <TrendingUp className="w-3.5 h-3.5 text-sky-500" />
            <span>Sinyal Pertandingan: Stabil</span>
          </div>
          <button
            onClick={() => setIsSlideMode((current) => !current)}
            className={`inline-flex items-center gap-1 px-3 py-1 rounded-full border text-[10px] font-semibold transition duration-200 ${
              isSlideMode
                ? 'bg-sky-600 text-white border-sky-600'
                : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-50'
            }`}
          >
            {isSlideMode ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
            {isSlideMode ? 'Slide Aktif' : 'Mulai Slide'}
          </button>
        </div>
      </div>
